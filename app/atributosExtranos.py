...
#Aplica el cierre
def cierre(cadenaI, listaL0X, listaL0Y, debug):
    cadenaF = str(cadenaI)
    if debug == 1:
...
def armar_cierre(list_a_cerrar, implicado, listaL0X, listaL0Y, debug):
    if debug == 1:
    ...
    for cadena in lista:
        cadena_cierre = cierre(str(cadena), listaL0X, listaL0Y, debug)
        if debug == 1:
